...
def end_screen():
    ...
    while running:
        ...
        for line in text_lines:
            ...
            line_rect = rendered_line.get_rect(center=(width// 2, y_offset))
...
def start_corsi_game(grid_size=3, sequence_length=3):
    # grid 
    margin = 50
    block_size = (width -100) // grid_size
    grid_positions = []
...
while running:
    ...
    if correct_answer:
        ...
        screen.blit(correct_text, correct_text.get_rect(center=(width//2, height//2)))
        pygame.display.update()
        pygame.time.delay(2000)
    else:
        screen.fill(WHITE)
        incorrect_text = TEXT_FONT.render("Incorrect, game over:(", True, BLACK)
        screen.blit(incorrect_text, incorrect_text.get_rect(center=(width//2, height//2)))
        pygame.display.update()
